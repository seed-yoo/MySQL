...
*/
select 
	   case when DAYOFWEEK(min(hire_date)) = 1 then date_format(min(hire_date), '%Y년 %m월 %d일 (일요일)')
			when DAYOFWEEK(min(hire_date)) = 2 then date_format(min(hire_date), '%Y년 %m월 %d일 (월요일)')
            when DAYOFWEEK(min(hire_date)) = 3 then date_format(min(hire_date), '%Y년 %m월 %d일 (화요일)')
            when DAYOFWEEK(min(hire_date)) = 4 then date_format(min(hire_date), '%Y년 %m월 %d일 (수요일)')
            when DAYOFWEEK(min(hire_date)) = 5 then date_format(min(hire_date), '%Y년 %m월 %d일 (목요일)')
            when DAYOFWEEK(min(hire_date)) = 6 then date_format(min(hire_date), '%Y년 %m월 %d일 (금요일)')
            when DAYOFWEEK(min(hire_date)) = 7 then date_format(min(hire_date), '%Y년 %m월 %d일 (토요일)')
			else ''
       END '날짜'
from employees;
